...
public abstract class GeneratorScreenTemplate<T extends GeneratorBlockEntityTemplate> extends HandledScreen {
    protected final PlayerInventory playerInventory;
    protected final T blockEntity;

    public GeneratorScreenTemplate(ScreenHandler screenHandler, PlayerInventory playerInventory, T blockEntity) {
        super(screenHandler);
    ...
    public void drawGauge(int x, int y, int mouseX, int mouseY) {
        ...
        if (size != null && net.glasslauncher.mods.glassguis.Rectangle.of(x, y, size[0], size[1]).contains(mouseX, mouseY)) {
    ...
    public void drawFluidGauge(int x, int y, int slot, int mouseX, int mouseY) {
        ...
        if (stack != null && stack.fluid != null) {
            ...
            int[] size = GlassGUIs.IMAGE_SIZE_CACHE.getIfPresent("/assets/glasstech/stationapi/textures/gui/energy_background.png");
            if (size != null && net.glasslauncher.mods.glassguis.Rectangle.of(offsetX, offsetY, size[0], size[1]).contains(mouseX, mouseY)) {
